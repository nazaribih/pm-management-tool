...
def list_tasks(status: Optional[models.TaskStatus] = None, project_id: Optional[int] = None, db: Session = Depends(get_db), user=Depends(get_current_user)):
    return crud.list_tasks(db, status=status, project_id=project_id)

@router.post("/", response_model=schemas.TaskOut, dependencies=[Depends(require_role(models.Role.user, models.Role.manager, models.Role.admin))])
...
@router.put("/{tid}", response_model=schemas.TaskOut, dependencies=[Depends(require_role(models.Role.user, models.Role.manager, models.Role.admin))])
