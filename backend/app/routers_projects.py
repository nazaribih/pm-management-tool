...
@router.post("/", response_model=schemas.ProjectOut, dependencies=[Depends(require_role(models.Role.manager, models.Role.admin))])
def create_project(data: schemas.ProjectCreate, db: Session = Depends(get_db)):
    return crud.create_project(db, data)

@router.put("/{pid}", response_model=schemas.ProjectOut, dependencies=[Depends(require_role(models.Role.manager, models.Role.admin))])
